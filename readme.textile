...
Firstly, you will need to copy the FeatureMirror.AddIn and FeatureMirror.dll into My Documents\Visual studio 2008\Addins\. Next you need to add a feature.yml file (normal yaml file) which contains one field, destination, which is a relative path from the feature.yml file to the destination of all files.
